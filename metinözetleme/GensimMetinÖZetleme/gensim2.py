...
def f(seq):  
    seen = set() 
    return [y for y in seq if y not in seen and not seen.add(y)] 
def summary(y, perc): 
    if len(split_sentences(y)) > 10:
        özet= summarize(y, ratio = perc, split=True)
        özet = '\n'.join(map(str, f(özet)))
    else:
        özet = y
    return özet
...
metin = """Front-line workers are getting some of the tech (and tracking) of office workers
...
ösatırlar=metin_özeti.splitlines()#satırlarına ayırma
...
print("Gensim Özetleme yüzdesi:",özet_oran)